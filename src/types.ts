...
export interface FraudAnalysis {
  ...
  immediateSolutions?: {
    ...
    title: string;
    description: string;
    actionType?: 'helpline' | 'report_portal' | 'bank_freeze' | 'mca_ticket' | 'none';
  }[];
  officialHelplines?: {
    name: string;
    contact: string;
...
export interface AppliedInterview {
  id: string;
  ...
  interviewerTimezone?: string; // e.g. "EST", "PST", "GMT", "IST"
  scheduledDateTime: string; // ISO 8601 string
  durationMinutes: number;
  locationType: 'AI_PORTAL_ROOM' | 'GOOGLE_MEET' | 'TEAMS' | 'IN_PERSON';
  locationDetail: string;
  meetingLink?: string;
  status: 'SCHEDULED' | 'COMPLETED' | 'UNDER_REVIEW' | 'INVITE_RECEIVED' | 'CANCELLED';
  score?: number;
  overallScore?: number;
  feedback?: string;
  interviewType: 'Technical' | 'HR' | 'System Architecture' | 'Comprehensive';
  difficulty: 'Beginner' | 'Intermediate' | 'Advanced';
  notes?: string;
}
