...
// Initialize Firestore with memory cache and forced long polling for maximum reliability across iframes, proxies, and preview containers
export const db = (() => {
  try {
...
async function testFirestoreConnection() {
  try {
    ...
    // Graceful offline fallback - Firestore automatically caches locally
    if (error instanceof Error && (error.message.includes('offline') || error.message.includes('unavailable') || error.message.includes('closing') || error.message.includes('hidden'))) {
      console.info('Firestore is operating with memory/cache persistence.');
    }
  }
}
