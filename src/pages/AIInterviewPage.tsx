...
export const AIInterviewPage: React.FC<AIInterviewPageProps> = ({ user, onNavigate }) => {
  const { t } = useLanguage();
  ...
  const getClientFallbackQuestions = (
    targetRole: string,
    targetCompany: string,
    count: number = 3,
    diff: string = 'Intermediate'
  ): AIInterviewQuestion[] => {
    const roleLower = targetRole.toLowerCase();
    const isFive = count === 5;
    const diffPrefix = diff === 'Advanced' ? '[System Architecture & Edge Cases] ' : diff === 'Basic' ? '[Foundations] ' : '';

    if (roleLower.includes('ai') || roleLower.includes('data') || roleLower.includes('machine learning') || roleLower.includes('data science')) {
      const qList: AIInterviewQuestion[] = [
        {
          id: 1,
          questionText: `${diffPrefix}Walk us through an end-to-end Machine Learning or Data Analytics project you have completed. What challenges did you face in data cleaning, feature engineering, and model optimization?`,
          userAnswer: '',
          feedback: '',
          score: 0
        },
        {
          id: 2,
          questionText: `${diffPrefix}How do you diagnose and prevent model overfitting, data drift, or algorithmic bias when preparing predictive models for enterprise workloads at ${targetCompany}?`,
          userAnswer: '',
          feedback: '',
          score: 0
        },
        {
          id: 3,
          questionText: `Under the PM Internship Scheme, how will you collaborate with domain specialists to deploy ethical, high-impact AI solutions into live enterprise operations?`,
          userAnswer: '',
          feedback: '',
          score: 0
        }
      ...
      if (isFive) {
        qList.push(
          {
            id: 4,
            questionText: `${diffPrefix}How would you architect an asynchronous data pipeline processing 500,000 real-time events per second using Kafka or stream processors?`,
            userAnswer: '',
            feedback: '',
            score: 0
          },
          {
            id: 5,
            questionText: `What is your long-term vision for utilizing AI governance and responsible innovation under India's AI mission during your internship at ${targetCompany}?`,
            userAnswer: '',
            feedback: '',
            score: 0
          }
      ...
      return qList;
    } else if (roleLower.includes('software') || roleLower.includes('full stack') || roleLower.includes('web') || roleLower.includes('developer') || roleLower.includes('frontend') || roleLower.includes('backend')) {
      const qList: AIInterviewQuestion[] = [
        {
          id: 1,
          questionText: `${diffPrefix}Explain how you architect scalable frontend applications in React with TypeScript and manage asynchronous state with REST or GraphQL APIs.`,
          userAnswer: '',
          feedback: '',
          score: 0
        },
        {
          id: 2,
          questionText: `${diffPrefix}Describe a difficult bug, memory leak, or performance bottleneck you resolved in production code. What was your systematic debugging process?`,
          userAnswer: '',
          feedback: '',
          score: 0
        },
        {
          id: 3,
          questionText: `How do you follow version control, automated unit testing, and collaborative code reviews when contributing to mission-critical codebases at ${targetCompany}?`,
          userAnswer: '',
          feedback: '',
          score: 0
        }
      ...
      if (isFive) {
        qList.push(
          {
            id: 4,
            questionText: `${diffPrefix}How do you structure microservices communication, database connection pooling, and caching with Redis for high concurrency?`,
            userAnswer: '',
            feedback: '',
            score: 0
          },
          {
            id: 5,
            questionText: `How will this software engineering role under the PM Internship Scheme help you build digital public infrastructure and enterprise systems?`,
            userAnswer: '',
            feedback: '',
            score: 0
          }
      ...
      return qList;
    } else if (roleLower.includes('ev') || roleLower.includes('battery') || roleLower.includes('auto') || roleLower.includes('mechanical') || roleLower.includes('mobility')) {
      const qList: AIInterviewQuestion[] = [
        {
          id: 1,
          questionText: `${diffPrefix}What are the core parameters monitored by a Battery Management System (BMS) in modern Electric Vehicles, and how do cell balancing and thermal runaway protections operate?`,
          userAnswer: '',
          feedback: '',
          score: 0
        },
        {
          id: 2,
          questionText: `${diffPrefix}How do you analyze regenerative braking efficiency and power transmission cycles in automotive test simulations for ${targetCompany}?`,
          userAnswer: '',
          feedback: '',
          score: 0
        },
        {
          id: 3,
          questionText: `What motivated you to apply for EV Engineering under the PM Internship Scheme, and how will you contribute to India's clean green mobility transition?`,
          userAnswer: '',
          feedback: '',
          score: 0
        }
      ...
      if (isFive) {
        qList.push(
          {
            id: 4,
            questionText: `${diffPrefix}Explain your approach to battery pack modular packaging, vibration damping, and fast-charging heat dissipation standards.`,
            userAnswer: '',
            feedback: '',
            score: 0
          },
          {
            id: 5,
            questionText: `Where do you see the biggest manufacturing and localization breakthroughs for indigenous EV components in the next 3 years?`,
            userAnswer: '',
            feedback: '',
            score: 0
          }
      ...
      return qList;
    } else if (roleLower.includes('cloud') || roleLower.includes('devops') || roleLower.includes('security') || roleLower.includes('cyber')) {
      const qList: AIInterviewQuestion[] = [
        {
          id: 1,
          questionText: `${diffPrefix}How do you set up automated CI/CD deployment pipelines using Docker containers and cloud infrastructure with strict security compliance?`,
          userAnswer: '',
          feedback: '',
          score: 0
        },
        {
          id: 2,
          questionText: `${diffPrefix}If a production service experiences sudden 502 bad gateway spikes or latency anomalies, what systematic telemetry steps do you take to identify the root cause?`,
          userAnswer: '',
          feedback: '',
          score: 0
        },
        {
          id: 3,
          questionText: `Why is cybersecurity hygiene and zero-trust IAM architecture essential for public and private enterprise cloud applications under the PM Scheme?`,
          userAnswer: '',
          feedback: '',
          score: 0
        }
      ...
      if (isFive) {
        qList.push(
          {
            id: 4,
            questionText: `${diffPrefix}How do you implement Infrastructure as Code (IaC) with Terraform and manage disaster recovery across multi-region clusters?`,
            userAnswer: '',
            feedback: '',
            score: 0
          },
          {
            id: 5,
            questionText: `How will your hands-on cloud skills at ${targetCompany} safeguard enterprise infrastructure against modern threat vectors?`,
            userAnswer: '',
            feedback: '',
            score: 0
          }
      ...
      return qList;
    } else if (roleLower.includes('fintech') || roleLower.includes('bank') || roleLower.includes('finance')) {
      const qList: AIInterviewQuestion[] = [
        {
          id: 1,
          questionText: `${diffPrefix}How do modern digital banking platforms handle real-time transaction reconciliation, ACID compliance, and UPI/IMPS payment processing at scale?`,
          userAnswer: '',
          feedback: '',
          score: 0
        },
        {
          id: 2,
          questionText: `${diffPrefix}Explain how automated fraud detection algorithms identify suspicious credit transactions while minimizing false positives for customers at ${targetCompany}.`,
          userAnswer: '',
          feedback: '',
          score: 0
        },
        {
          id: 3,
          questionText: `How will your technical skills in financial technology support digital financial inclusion under the PM Internship Scheme?`,
          userAnswer: '',
          feedback: '',
          score: 0
        }
      ...
      if (isFive) {
        qList.push(
          {
            id: 4,
            questionText: `${diffPrefix}What strategies would you employ to guarantee 99.999% uptime for core banking ledger APIs during peak festive sales?`,
            userAnswer: '',
            feedback: '',
            score: 0
          },
          {
            id: 5,
            questionText: `How do you envision regulatory compliance (RBI guidelines) integrating seamlessly into agile financial software development?`,
            userAnswer: '',
            feedback: '',
            score: 0
          }
      ...
      return qList;
    } else if (roleLower.includes('supply') || roleLower.includes('logistics') || roleLower.includes('operations')) {
      const qList: AIInterviewQuestion[] = [
        {
          id: 1,
          questionText: `${diffPrefix}How do you leverage route optimization algorithms and IoT sensors to improve last-mile supply chain visibility and reduce freight transit times?`,
          userAnswer: '',
          feedback: '',
          score: 0
        },
        {
          id: 2,
          questionText: `${diffPrefix}Describe how predictive demand forecasting prevents inventory stockouts and reduces warehouse holding costs for enterprise distribution at ${targetCompany}.`,
          userAnswer: '',
          feedback: '',
          score: 0
        },
        {
          id: 3,
          questionText: `How do you handle unexpected vendor delays or supply disruptions while maintaining strict SLA commitments under the PM Internship Scheme?`,
          userAnswer: '',
          feedback: '',
          score: 0
        }
      ...
      if (isFive) {
        qList.push(
          {
            id: 4,
            questionText: `${diffPrefix}Explain the principles of Just-In-Time (JIT) manufacturing and how ERP systems orchestrate multi-tiered supplier nodes.`,
            userAnswer: '',
            feedback: '',
            score: 0
          },
          {
            id: 5,
            questionText: `What key operational metric would you target to improve by 15% during your internship at ${targetCompany}?`,
            userAnswer: '',
            feedback: '',
            score: 0
          }
      ...
      const qList: AIInterviewQuestion[] = [
        {
          id: 1,
          questionText: `${diffPrefix}Walk us through a technical project or academic scenario where you applied core skills relevant to ${targetRole}. What was your specific responsibility and measurable outcome?`,
          userAnswer: '',
          feedback: '',
          score: 0
        },
        {
          id: 2,
          questionText: `${diffPrefix}How do you approach learning unfamiliar technologies or troubleshooting unexpected problems when working under tight deadlines in the PM Internship Scheme?`,
          userAnswer: '',
          feedback: '',
          score: 0
        },
      ...
      if (isFive) {
        qList.push(
          {
            id: 4,
            questionText: `${diffPrefix}Describe an instance where you worked with cross-disciplinary team members to solve a complex practical challenge.`,
            userAnswer: '',
            feedback: '',
            score: 0
          },
          {
            id: 5,
            questionText: `How do you plan to leverage the PM Internship Scheme mentorship to excel in your technical domain over the next year?`,
            userAnswer: '',
            feedback: '',
            score: 0
          }
  ...
  const startInterview = async () => {
    ...
    // 1. Instantly construct high-quality domain-specific session (< 20ms)
    const instantQuestions = getClientFallbackQuestions(targetRole, targetCompany, questionCount, difficulty);
    const instantSession: AIInterviewSession = {
    ...
    // Immediately enter the interview room without waiting or blocking UI
    setSession(instantSession);
    setLoading(false);

    // 2. Optional fast background enrichment with short 900ms timeout
    try {
        ...
        if (textData) {
          ...
          if (data && data.questions && data.questions.length > 0) {
            setSession((prev) => {
              ...
              if (prev && prev.currentQuestionIndex === 0 && !prev.questions[0]?.userAnswer) {
                return { ...prev, ...data };
              }
              return prev;
            });
      ...
      // Instant session is already active and running flawlessly
    }
  };
              ...
              {onNavigate && (
                <>
                  <button
                    ...
                    className="px-4 py-3 bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold text-xs rounded-xl flex items-center gap-1.5 transition shadow-xs cursor-pointer"
                  >
                    <CheckCircle2 className="w-4 h-4" />
                    <span>Apply to {company} Roles →</span>
                  </button>
                  <button
                    onClick={() => onNavigate('ai-skill-gap')}
                    className="px-4 py-3 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 font-bold text-xs rounded-xl flex items-center gap-1.5 transition cursor-pointer"
                  >
