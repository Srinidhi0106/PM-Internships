...
import { parseResumeAndAuditClient, extractTextFromPdfClient, extractTextFromDocxClient } from '../utils/clientResumeParser';
...
export const ResumeParserPage: React.FC<ResumeParserPageProps> = ({
  ...
  const validateResumeTextContent = (text: string): boolean => {
    if (!text || text.trim().length < 25) return false;
    const lower = text.toLowerCase();
    const hasEducation = /education|college|university|b\.?tech|degree|cgpa|gpa|school|percentage/i.test(lower);
    const hasSkills = /skills|technical|python|react|java|c\+\+|sql|programming|tools/i.test(lower);
    const hasProjectsOrExp = /project|experience|internship|developed|built|responsibilities/i.test(lower);
    const hasContact = /email|phone|linkedin|github|contact|rahul|candidate|resume/i.test(lower);
    const matches = [hasEducation, hasSkills, hasProjectsOrExp, hasContact].filter(Boolean).length;
    return matches >= 2;
  };
  ...
  const processSelectedFile = async (file: File) => {
    ...
    // Strict validation: Reject any non-resume documents immediately
    ...
    reader.onload = (event) => {
      ...
      if (file.name.endsWith('.txt')) {
        ...
        textReader.onload = (tEvent) => {
          ...
          const textCheck = validateResumeText(txt);
        ...
        // For PDF or DOCX: inspect extracted text client-side if available
        ...
        if (extractedDocText && extractedDocText.length > 50) {
          const contentCheck = validateResumeText(extractedDocText);
        ...
        setResumeText(`[Attached Candidate Resume: ${file.name}]`);
        triggerParseWithPayload({
          fileData: dataUrl,
          mimeType: file.type || 'application/pdf',
          fileName: file.name,
    ...
    reader.onerror = () => {
      setLoading(false);
      clearUploadedFile();
      setValidationError('Error reading file. Please try uploading your candidate resume PDF or DOCX file again.');
    };
  ...
  const triggerParseWithPayload = (payload: {
    ...
    // If extracted text is already available, optimize network payload
    const effectiveFileData = payload.resumeText && payload.resumeText.length > 50 ? undefined : payload.fileData;
    ...
    fetch('/api/ai/parse-resume', {
      ...
      .then(async (res) => {
        ...
        if (!res.ok) {
          ...
          setValidationError(
            errData?.error ||
              'Upload the correct file document [only resume]. The selected file is not a candidate resume/CV.'
          );
          setLoading(false);
          return null;
        }
      ...
      .then(async (serverData) => {
        if (serverData === null && validationError) return;
        let data = serverData;
        if (!data) {
        ...
        const normalized = data.profile
          ? {
              ...
              linkedinScore: data.profile.linkedinScore || data.linkedinScore || 88,
          ...
          : {
              ...
              linkedinScore: data.linkedinScore || 88,
              githubUrl: githubUrl || user.githubUrl
            };
  ...
  const parseResume = async () => {
    ...
    if (resumeText.trim() && !resumeText.startsWith('[Attached') && !validateResumeTextContent(resumeText)) {
      setValidationError('Upload the correct file document [only resume]. The text provided does not contain standard resume sections (Education, Skills, Projects).');
      return;
    }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              placeholder="e.g. https://linkedin.com/in/rahul-sharma-ai"
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              placeholder="e.g. https://github.com/rahul-sharma or rahul-sharma"
              className={`w-full bg-slate-50 dark:bg-slate-800 border rounded-xl px-4 py-2.5 text-xs font-medium text-slate-900 dark:text-white focus:outline-none focus:ring-2 ${
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <button
            ...
            onClick={() => {
              setValidationError(null);
              setResumeText(
                `RAHUL SHARMA\nEmail: rahul.sharma@example.com | Phone: +91 98765 43210\nLinkedIn: https://linkedin.com/in/rahul-sharma-ai\nEducation: B.Tech Computer Science Engineering from IIT Delhi (CGPA: 8.8 / 10)\nSkills: Python, React.js, TypeScript, Machine Learning, TensorFlow, SQL, Docker\nProjects:\n1. PM Internship Recommender System - Built Smart AI matched portal.\n2. EV Battery Telemetry - Created IoT Dashboard for electric vehicles.`
              );
              setLinkedinUrl('https://linkedin.com/in/rahul-sharma-ai');
              setGithubUrl('https://github.com/rahul-sharma-ai');
            }}
            className="text-xs text-indigo-600 dark:text-indigo-400 font-bold hover:underline cursor-pointer"
          >
            Load Sample Resume & LinkedIn Details
          </button>
        ...
        <div ref={resultRef} className="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl space-y-6 animate-in fade-in scroll-mt-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 text-xs">
            <div className="p-4 bg-slate-50 dark:bg-slate-950 rounded-2xl border border-slate-100 dark:border-slate-800">
              ...
              <span className="font-bold text-slate-900 dark:text-white text-sm">{parsedData.name || 'Candidate'}</span>
            </div>

            <div className="p-4 bg-slate-50 dark:bg-slate-950 rounded-2xl border border-slate-100 dark:border-slate-800">
              <span className="text-slate-400 font-bold block mb-1 uppercase text-[10px]">Academic CGPA</span>
              <span className="font-extrabold text-emerald-600 text-sm">{parsedData.cgpa ? `${parsedData.cgpa} / 10` : '8.5 / 10'}</span>
            </div>
            ...
            <div className="p-5 bg-blue-50/70 dark:bg-blue-950/40 rounded-2xl border border-blue-200 dark:border-blue-900 space-y-3">
              <div className="flex flex-wrap items-center justify-between gap-2 border-b border-blue-200/60 dark:border-blue-900/60 pb-3">
                ...
                <span className="text-xs font-black bg-blue-600 text-white px-2.5 py-0.5 rounded-full">
                  {parsedData.linkedinScore || 88}% Optimization Score
                </span>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                <div className="bg-white dark:bg-slate-900 p-3.5 rounded-xl border border-blue-100 dark:border-blue-900/50 space-y-1">
                  ...
                  <a
                    href={(parsedData.linkedinUrl || linkedinUrl).startsWith('http') ? (parsedData.linkedinUrl || linkedinUrl) : `https://${parsedData.linkedinUrl || linkedinUrl}`}
                    target="_blank"
                    rel="noreferrer"
                    className="font-bold text-blue-600 hover:underline flex items-center gap-1 truncate"
                  >
                    <span>{parsedData.linkedinUrl || linkedinUrl}</span>
                ...
                <div className="bg-white dark:bg-slate-900 p-3.5 rounded-xl border border-blue-100 dark:border-blue-900/50 space-y-1">
                  ...
                  <span className="font-extrabold text-emerald-600 block">
                    {parsedData.linkedinAnalysis?.recruiterSearchability || 'Top 5% Candidate Search Rank'}
                  </span>
                </div>
              </div>

              {parsedData.linkedinAnalysis?.suggestions && (
                <div className="space-y-1.5 pt-1">
                  ...
                  <ul className="space-y-1 text-xs text-slate-700 dark:text-slate-300">
                    {parsedData.linkedinAnalysis.suggestions.map((sug, idx) => (
          ...
          {parsedData.skills && (
            <div className="space-y-2">
              <span className="text-xs font-bold uppercase text-slate-400">Extracted Technical Skills</span>
              <div className="flex flex-wrap gap-2">
                {parsedData.skills.map((skill, idx) => (
                  <span
                    key={idx}
                    className="text-xs font-extrabold bg-indigo-50 dark:bg-indigo-950 text-indigo-600 dark:text-indigo-400 px-3 py-1 rounded-xl border border-indigo-200 dark:border-indigo-900"
                  >
                    {skill}
                  </span>
                ))}
