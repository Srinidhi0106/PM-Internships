...
export const AIPortfolioPage: React.FC<AIPortfolioPageProps> = ({ user }) => {
  ...
  const processSelectedFile = async (file: File) => {
    ...
    reader.onload = async () => {
      ...
      try {
        // Fast client-side text extraction (< 5ms)
        ...
        // If text was extracted, run fast strict client content validation
        ...
        try {
          ...
          const verifyRes = await fetch('/api/ai/parse-resume', {
            ...
            body: JSON.stringify({
              fileData: extractedDocText && extractedDocText.length > 50 ? undefined : base64,
          ...
          if (verifyRes.ok) {
            ...
            setValidationError(
              errData?.error ||
                `Upload the correct file document [only resume]. The selected file "${file.name}" is not a valid candidate resume/CV.`
            );
            setVerifyingFile(false);
            return;
          }
        ...
        // If server was unreachable, use client-side parser
        if (!verifyData) {
    ...
    try {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setValidationError(
            errData?.error ||
              'Upload the correct file document [only resume]. The provided document was rejected as it is not a candidate resume/CV.'
          );
          setLoading(false);
          return;
        }
      ...
      // If server was unreachable, use client-side audit engine
      if (!data) {
    ...
    try {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const errData = await res.json().catch(() => null);
          setValidationError(errData?.error || 'Upload the correct file document [only resume].');
          setRecheckingAts(false);
          return;
        }
      } catch {
      ...
      <div className="bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-md space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              placeholder="e.g. rahul-sharma or https://github.com/rahul-sharma"
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2">
          <button
            ...
            onClick={() => {
              setValidationError(null);
              setGithubUsername('rahul-sharma-ai');
              setLinkedinUrl('https://linkedin.com/in/rahul-sharma-ai');
              setResumeText(
                'RAHUL SHARMA\nEducation: B.Tech Computer Science from IIT Delhi (CGPA: 8.8/10)\nSkills: Python, React.js, TypeScript, Machine Learning, TensorFlow, SQL, Docker\nProjects:\n1. PM Internship AI Matching Platform - Built candidate entity extractor.\n2. EV Battery Telemetry - Created IoT Dashboard.'
              );
            }}
            className="text-xs text-indigo-600 dark:text-indigo-400 font-bold hover:underline cursor-pointer"
          >
            Load Sample Profile & Resume Details
          </button>
