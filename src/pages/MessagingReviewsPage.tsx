...
// Helper to generate dynamic relative timestamps based on current system time
const getDynamicTimeStr = (minutesAgo: number): string => {
...
const getDynamicDateStr = (daysAgo: number, timeStr: string): string => {
...
const buildInitialConversations = (): Conversation[] => [
  {
    id: 'conv-tcs',
    ...
    messages: [
      {
        ...
        timestamp: getDynamicTimeStr(28),
        status: 'read'
      },
      {
        id: 'm2',
        ...
        timestamp: getDynamicTimeStr(25),
        status: 'read'
      },
      {
        id: 'm3',
        ...
        timestamp: getDynamicTimeStr(18),
        status: 'read'
      },
      {
        id: 'm4',
        ...
        timestamp: getDynamicTimeStr(12),
        status: 'read'
      }
    ]
  },
    ...
    messages: [
      {
        ...
        timestamp: getDynamicDateStr(1, '04:15 PM'),
        status: 'read'
      },
      {
        id: 'm-tm-2',
        ...
        timestamp: getDynamicDateStr(1, '04:30 PM'),
        status: 'read'
      },
      {
        id: 'm-tm-3',
        ...
        timestamp: getDynamicDateStr(1, '04:45 PM'),
        status: 'read'
      }
    ]
  },
    ...
    messages: [
      {
        ...
        timestamp: getDynamicDateStr(2, '11:20 AM'),
        status: 'read'
      }
    ]
  },
    ...
    messages: [
      {
        ...
        timestamp: getDynamicDateStr(3, '02:40 PM'),
        status: 'read'
      }
    ]
  },
    ...
    messages: [
      {
        ...
        timestamp: getDynamicDateStr(4, '10:00 AM'),
        status: 'read'
      }
    ]
  }
...
export const MessagingReviewsPage: React.FC<MessagingReviewsPageProps> = ({ user }) => {
  ...
  // Conversations State with localStorage fallback and live timestamp generator
  const [conversations, setConversations] = useState<Conversation[]>(() => {
    try {
      const saved = localStorage.getItem('pm_portal_conversations');
      if (saved) {
    ...
    return buildInitialConversations();
  });
  ...
  useEffect(() => {
    try {
      localStorage.setItem('pm_portal_conversations', JSON.stringify(conversations));
    } catch (e) {
  ...
  const handleResetCurrentChat = () => {
    const freshConvs = buildInitialConversations();
    setConversations(freshConvs);
    localStorage.removeItem('pm_portal_conversations');
    setToastMessage('Conversation thread refreshed with live current timestamp!');
  ...
  const handleSendMessage = (customText?: string) => {
    ...
    const currentFormattedTime = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true });
    ...
    setTimeout(() => {
      ...
      } else if (lower.includes('resume') || lower.includes('cv') || lower.includes('portfolio') || lower.includes('attached')) {
      ...
      const replyFormattedTime = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true });

      const incomingReply: ChatMessage = {
        id: `reply-${Date.now()}`,
        sender: selectedConv.id === 'conv-mca' ? 'mca' : 'company',
        text: replyText,
        timestamp: replyFormattedTime,
        status: 'read'
      };
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
          ...
          <div className="lg:col-span-8 flex flex-col h-full min-h-[580px]">
            ...
            <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-white dark:bg-slate-900">
              ...
              <div className="flex items-center gap-2">
                <div className="hidden sm:flex items-center gap-1.5 bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800 text-[10px] font-bold px-2.5 py-1 rounded-full">
                  <span className="w-2 h-2 rounded-full bg-emerald-500 animate-ping inline-block" />
                  <span>Real-Time Sync Active</span>
                </div>

                <span className="hidden md:inline-flex bg-amber-50 dark:bg-amber-950/60 text-amber-800 dark:text-amber-300 border border-amber-200 dark:border-amber-800 text-[10px] font-bold px-2.5 py-1 rounded-full">
                ...
                <button
                  ...
                  title="Reset to fresh live timestamp session"
                  className="p-1.5 text-slate-400 hover:text-purple-600 hover:bg-purple-50 dark:hover:bg-slate-800 rounded-lg transition text-xs font-semibold flex items-center gap-1 cursor-pointer border border-transparent hover:border-purple-200"
                >
                  <RotateCcw className="w-3.5 h-3.5" />
                  <span className="hidden sm:inline text-[10px]">Reset Chat</span>
                </button>
              </div>
            </div>

            {toastMessage && (
              <div className="mx-4 mt-2 p-2.5 bg-emerald-600 text-white text-xs font-bold rounded-xl text-center shadow-md animate-fade-in flex items-center justify-center gap-2">
      ...
      {activeTab === 'reviews' && (
        <div className="space-y-6">
          ...
          {/* AI NLP Sentiment Analysis & Theme Extraction Panel (Slide 8 Requirement) */}
          <div className="bg-gradient-to-r from-purple-500/10 via-indigo-500/10 to-emerald-500/10 border border-purple-200 dark:border-purple-900/60 p-6 rounded-3xl space-y-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-sm font-black text-slate-900 dark:text-white flex items-center gap-1.5">
                    <span>AI Sentiment Analysis of Past Internship Reviews</span>
                  ...
                  <p className="text-[11px] text-slate-500 font-medium">
                    Automated sentiment polarity and aspect-based thematic extraction from 1,420+ candidate testimonials.
                  </p>
                </div>
              </div>

              <div className="flex items-center gap-3 text-xs font-bold">
                <span className="text-emerald-600 dark:text-emerald-400">91% Positive</span>
                <span className="text-slate-400">•</span>
                <span className="text-slate-500">7% Neutral</span>
                <span className="text-slate-400">•</span>
                <span className="text-amber-600">2% Constructive</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
              <div className="p-3 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-1">
                <span className="text-[10px] font-bold text-slate-400 uppercase">Mentorship & Guidance</span>
                <div className="flex items-center justify-between">
                  <span className="font-extrabold text-emerald-600">95% Favorable</span>
              ...
              <div className="p-3 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-1">
                <span className="text-[10px] font-bold text-slate-400 uppercase">On-Time DBT Stipend</span>
                <div className="flex items-center justify-between">
                  <span className="font-extrabold text-emerald-600">98% Verified</span>
              ...
              <div className="p-3 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-1">
                <span className="text-[10px] font-bold text-slate-400 uppercase">Skill Growth & Learning</span>
                <div className="flex items-center justify-between">
                  <span className="font-extrabold text-indigo-600">92% High Gain</span>
              ...
              <div className="p-3 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-1">
                <span className="text-[10px] font-bold text-slate-400 uppercase">PPO Conversion Rate</span>
                <div className="flex items-center justify-between">
                  <span className="font-extrabold text-purple-600">87% Placement</span>
                  <span className="text-[10px] text-slate-400">4.7★</span>
                </div>
                <div className="w-full h-1.5 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
